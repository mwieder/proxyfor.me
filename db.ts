...
function createbots() {
    ...
    for (i = 0; i < nbots; i++) {
        ...
        for (j = 0; j < ni; j++) {
            botprofiles[i].ideology += String.fromCharCode(48 + flip(4));;
        }
        setpw(botprofiles[i]);
    }
    ...
    botprofiles[10].contact = botprofiles[80].contact = botprofiles[110].contact = botprofiles[180].contact = "e";
    ...
    botprofiles[nbots].ideology = lotsazeros.substr(0, ni);
    ...
    stream.on('end', () => {
        ...
        stream.on('data', (v: amVote) => {
            ...
            fs.writeSync(fd, oid.getTimestamp().getTime() + tab + v.screen_name + tab + v.vote + tab
                + v.big5 + tab + v.ideology + tab + v.ip + nl);
            if (v.contact === "e") {
                ...
                if (!p)
                    ...
                    if (token !== p.token)
                        ...
                        for (let f in profileSetSpec)
                            p[f] = profile[f];
                        db.collection("Profile").updateOne({ _id: p._id }, p)
                            .then(r => resolve(null))
                            .catch(e => reject(e));
                    }
            })
        ...
        db.collection("Profile").findOne({ token: t })
            .then(voter => {
                ...
                db.collection("DirectVote").updateOne(
                    { screen_name: vote.screen_name, proposal: vote.proposal }, vote, { upsert: true, w: 1 })
                    .then(r => {
        ...
        db.collection("Profile").findOne({ token: t })
            .then(voter => {
                ...
                if (p.category === "q") {
                    ...
                    delete p._id;
                    db.collection("Post").updateOne({ proposal: p.proposal, screen_name: p.screen_name }, p, { upsert: true })
                        .then(r => resolve(null))
                        .catch(e => reject(e));
                }
            })
    ...
    return new Promise((resolve, reject) => {
        db.collection("Profile").findOne({ token: t })
            .then(citer => {
                ...
                if (add)
                    db.collection("Cite").updateOne(c, c, { upsert: true })
                        .then((r) => {
            ...
            .then(p => {
                if (!p) {
                    if (li.email) {
                        ...
                        p.ideology = lotsazeros.substr(0, ni);
                    }
                    else
                        return resolve(null);
                }
                ...
                if (p.hash) // existing profile login check
                    ...
                    p.token = jwt.sign({ token: p.screen_name }, p.salt);
                    db.collection("Profile").updateOne({ screen_name: p.screen_name }, p, { upsert: true })
